...
RPC_URL = os.environ.get("RPC_URL", "http://127.0.0.1:9982")
...
def tx_compute_hash(tx):
    ...
    #return zny_yespower_0_5.getPoWHash(bytes.fromhex(tx))[::-1].hex()
...
def block_compute_raw_hash(header):
    ...
    return zny_yespower_0_5.getPoWHash(header)[::-1]
...
def block_mine(block_template, coinbase_message, extranonce_start, address, timeout=None, debugnonce_start=False):
    ...
    while extranonce <= 0xffffffff:
        ...
        while nonce <= 0xffffffff:
            ...
            if nonce > 0 and nonce % 1024 == 0:
                hash_rate = hash_rate + ((1024 / (time.time() - time_stamp)) - hash_rate) / (hash_rate_count + 1)
                hash_rate_count += 1
...
def standalone_miner(coinbase_message, address):
    while True:
        ...
        mined_block, hash_rate = block_mine(block_template, coinbase_message, 0, address, timeout=180)
